...
void QtTreePropertyBrowserPrivate::propertyInserted(QtBrowserItem *index, QtBrowserItem *afterIndex)
{
	...
	if (index->property()->isEditorHold())
	{
...
{
	d_ptr->m_treeWidget->setColumnCount(labels.size());
	d_ptr->m_treeWidget->setHeaderLabels(labels);
}

void QtTreePropertyBrowser::setContextMenu(QAction *action)
{
	d_ptr->m_treeWidget->setSelectionMode(QAbstractItemView::SingleSelection); //设置选择模式
	d_ptr->m_treeWidget->setContextMenuPolicy(Qt::ActionsContextMenu);         //设置菜单模式
	d_ptr->m_treeWidget->addAction(action);
}

void QtTreePropertyBrowser::clearContextMenu()
{
	QList<QAction *> lstAct = d_ptr->m_treeWidget->actions();
	for (auto rit = lstAct.crbegin(), rend = lstAct.crend(); rit != rend; ++rit)
	{
		d_ptr->m_treeWidget->removeAction(*rit);
	}
}

QModelIndex QtTreePropertyBrowser::currentIndex() const
{
	return d_ptr->m_treeWidget->selectionModel()->currentIndex();
}
