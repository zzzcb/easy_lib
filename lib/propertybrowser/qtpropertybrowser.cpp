...
{
public:
    QtPropertyPrivate(QtAbstractPropertyManager *manager) : m_enabled(true), m_modified(false), m_editorHold(false),m_manager(manager) {}
    QtProperty *q_ptr;
...
bool QtProperty::isEditorHold() const
{
	return d_ptr->m_editorHold;
}
...
void QtProperty::setEditorHold(bool hold)
{
	if (d_ptr->m_editorHold == hold)
		return;

	d_ptr->m_editorHold = hold;
	propertyChanged();
}
